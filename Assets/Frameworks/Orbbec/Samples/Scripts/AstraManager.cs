...
public class AstraManager : Singleton<AstraManager>
{
...
#else
            int targetDepthWidth = 640;
            int targetDepthHeight = 480;
            int targetDepthFps = 30;
#endif
    ...
    public void UpdateBody(BodyFrame bodyFrame)
    {
        if (bodyFrame == null)
        {
            Debug.Log("No Bodies Available!");
            return;
        }
        //Debug.Log("Bodies Available!");
        bodyFrame.CopyBodyData(ref _bodies);
    }
    #endregion


    public int GetUserIndexById(byte id)
    {
        int counter = 0;
        foreach (Body body in Bodies)
        {
    ...
    public Astra.Joint GetJointByType(Body body, JointType type)
    {
        if (body != null && body.Joints != null)
        {
            
            return body.Joints[(int)type];
        }
        else
        {
            return null;
