	...
	void Start() 
	{
		try 
		{
			...
			if(foregroundCamera != null)
			{
				...
				float rectHeight = neededFgRect.width;
				float rectWidth = neededFgRect.width * rectHeight / neededFgRect.height;
				foregroundRect = new Rect(- rectWidth / 2, rectHeight, rectWidth, -rectHeight);
		...
		if(isBrInited)
		{
			...
			if(bSuccess)
			{
				...
				if(kinectManager && kinectManager.IsInitialized())
				{
					...
					foregroundTex.Apply();
					DistanceBody.texture = sensorData.color2DepthTexture;
					if (!Settings.Instance.ShowBackground)
					{
						DisplayImage.texture = sensorData.color2DepthTexture; 
					}
				}
			}
		}
