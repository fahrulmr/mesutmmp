...
public class InteractionManager : MonoBehaviour
{
	...
	public float SmoothFactor
	{
		get
		{
			return PanelManager.Instance.GetPanel<SettingsPanelBehaviour>().Settings.SmoothFactor;
		}
	}
	...
	public float CoordinateScaler = 1.0f;
	...
	private List<Vector3> leftHandScreenPosLast = new List<Vector3>();
	...
	private List<Vector3> rightHandScreenPosLast = new List<Vector3>();
	...
	{
		return !isLeftHandPrimary && !isRightHandPrimary;
	}
	...
	void Start()
	{
		...
		HandScale = PanelManager.Instance.GetPanel<SettingsPanelBehaviour>().Settings.HandScale;
		CoordinateScaler = PanelManager.Instance.GetPanel<SettingsPanelBehaviour>().Settings.CursorSpeed;
...
//					monoScript.enabled)
	...
	Vector3 GetSmoothedPosition(Vector3 pos, List<Vector3> lastPositions)
	{
		Vector3 smoothedPos = pos;
//		lastPositions.Add (pos);
		...
		smoothedPos /= (lastPositions.Count + 1);
		lastPositions.Add((smoothedPos + pos) * 0.5f);
		return smoothedPos;
	}
	...
	void Update()
	{
		...
		{
			playerUserID = playerTracker.PlayerUserID;
			playerIndex = playerTracker.PlayerIndex;
			if (playerUserID != 0)
			{
...
//					leftHandScreenPos.z = Mathf.Clamp01((leftIboxLeftBotBack.z - leftHandPos.z) / (leftIboxLeftBotBack.z - leftIboxRightTopFront.z));
					...
					Vector3 offset = new Vector3(0.5f, 0.25f, 0.0f);

					leftHandScreenPos.x = (leftHandPos.x - leftIboxLeftBotBack.x) / (leftIboxRightTopFront.x - leftIboxLeftBotBack.x);
					leftHandScreenPos.x = (leftHandScreenPos.x - offset.x) * CoordinateScaler + offset.x;
					leftHandScreenPos.x = Mathf.Clamp01(leftHandScreenPos.x);

					leftHandScreenPos.y = (leftHandPos.y - leftIboxLeftBotBack.y) / (leftIboxRightTopFront.y - leftIboxLeftBotBack.y);
					leftHandScreenPos.y = (leftHandScreenPos.y - offset.y) * CoordinateScaler + offset.y;
					leftHandScreenPos.y = Mathf.Clamp01(leftHandScreenPos.y);

					leftHandScreenPos.z = (leftIboxLeftBotBack.z - leftHandPos.z) / (leftIboxLeftBotBack.z - leftIboxRightTopFront.z);
					leftHandScreenPos.z = (leftHandScreenPos.z - offset.z) * CoordinateScaler + offset.z;
					leftHandScreenPos.z = Mathf.Clamp01(leftHandScreenPos.z);

					if (UseCameraProjection)
					{
						var overlayer = GetComponent<JointScreenOverlayer>();
						if (overlayer != null)
						{
							//leftHandScreenPos = overlayer.GetScreenPosition(playerUserID, (int)KinectInterop.JointType.HandLeft, leftHandScreenPos);
							Vector3 center = new Vector3(0.5f, 0.5f, 0.0f);
							leftHandScreenPos = (leftHandScreenPos - center) * CoordinateScaler + center;
							leftHandScreenPos = GetSmoothedPosition(leftHandScreenPos, leftHandScreenPosLast);
//							leftHandScreenPos = (leftHandScreenPos - center) * kinectManager.GetColorImageWidth() / Screen.width + center;
						}
					}
...
//					rightHandScreenPos.z = Mathf.Clamp01((rightIboxLeftBotBack.z - rightHandPos.z) / (rightIboxLeftBotBack.z - rightIboxRightTopFront.z));
					...
					rightHandScreenPos.x = (rightHandPos.x - rightIboxLeftBotBack.x) / (rightIboxRightTopFront.x - rightIboxLeftBotBack.x);
					rightHandScreenPos.x = (rightHandScreenPos.x - offset.x) * CoordinateScaler + offset.x;
					rightHandScreenPos.x = Mathf.Clamp01(rightHandScreenPos.x);

					rightHandScreenPos.y = (rightHandPos.y - rightIboxLeftBotBack.y) / (rightIboxRightTopFront.y - rightIboxLeftBotBack.y);
					rightHandScreenPos.y = (rightHandScreenPos.y - offset.y) * CoordinateScaler + offset.y;
					rightHandScreenPos.y = Mathf.Clamp01(rightHandScreenPos.y);

					rightHandScreenPos.z = (rightIboxLeftBotBack.z - rightHandPos.z) / (rightIboxLeftBotBack.z - rightIboxRightTopFront.z);
					rightHandScreenPos.z = (rightHandScreenPos.z - offset.z) * CoordinateScaler + offset.z;
					rightHandScreenPos.z = Mathf.Clamp01(rightHandScreenPos.z);
					...
					if (UseCameraProjection)
					{
						...
						if (overlayer != null)
						{
							//rightHandScreenPos = overlayer.GetScreenPosition(playerUserID, kinectManager.GetJointByType(kinectManager.Bodies[playerUserID], Astra.JointType.RightHand), rightHandScreenPos);
							Vector3 center = new Vector3(0.5f, 0.5f, 0.0f);
							rightHandScreenPos = (rightHandScreenPos - center) * CoordinateScaler + center;
							rightHandScreenPos = GetSmoothedPosition(rightHandScreenPos, rightHandScreenPosLast);
//							rightHandScreenPos = (rightHandScreenPos - center) * kinectManager.GetColorImageWidth () / Screen.width + center;
...
//				}
		...
		if (showHandCursor)
		{
			...
			//if(useHandCursor)
			{
				...
				{
					Rect rectTexture; 
					float scaler = Screen.height * HandScale / 600.0f;
					Vector2 size = new Vector2(texture.width, texture.height) * scaler;
//					if(controlMouseCursor)
//					{
					MouseControl.MouseMove(cursorScreenPos, debugText);
					rectTexture = new Rect(Input.mousePosition.x - size.x / 2, Screen.height - Input.mousePosition.y - size.y / 2, 
						size.x, size.y);
//					}
//					else 
//					{
